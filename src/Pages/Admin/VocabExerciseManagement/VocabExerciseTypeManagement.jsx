...
const VocabExerciseTypeManagement = () => {
  ...
  const fetchTypes = async () => {
    try {
      setLoading(true);
      const response = await adminService.getAllVocabExerciseTypes(currentPage, 10);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name) {
      toast.error('Vui lòng nhập tên loại bài tập');
      return;
    }

    try {
      if (modalMode === 'add') {
        const response = await adminService.createVocabExerciseType(formData);
        ...
        const response = await adminService.updateVocabExerciseType(selectedType.id, formData);
        if (response.code === 1000) {
  ...
  const handleDeleteRestore = async (typeId, isActive) => {
    ...
    try {
      const response = await adminService.deleteOrRestoreVocabExerciseType(typeId, status);
      if (response.code === 1000) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">Loại Bài Tập Từ Vựng</h2>
          ...
          <table className="min-w-full bg-white border">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Hướng dẫn</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredTypes.map((type) => (
                <tr key={type.id} className={!type.isActive ? 'bg-gray-100' : ''}>
                  ...
                  <td className="px-6 py-4 text-sm">{type.instruction || '-'}</td>
...
export default VocabExerciseTypeManagement;
